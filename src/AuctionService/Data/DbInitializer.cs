...
{
    //khởi tạo phương thức tĩnh để sử dụng trong các lớp khác mà ko cần khởi tạo mới lớp DbInitializer 
    public static void InitDb(WebApplication app){
        using var scope = app.Services.CreateScope(); //tạo 1 phạm vi để quản lý vòng đời của các dịch vụ được đăng ký trong IServiceCollection của Service Provider 
        //vì sao cần phải tạo phạm vi đê sử dụng dịch vụ AuctionDbContext?
        //vì AuctionDbContext được đăng ký trong IServiceCollection của Service Provider nên cần phải tạo phạm vi để sử dụng được dịch vụ này
        SeedData(scope.ServiceProvider.GetService<AuctionDbContext>()); //gọi được AuctionDbContext vì đã đăng ký trong Program.cs(đây gọi là DI)
    }

    private static void SeedData(AuctionDbContext context)
    {
        context.Database.Migrate(); //tạo database nếu chưa tồn tại, nếu tồn tại thì update schema nếu có thay đổi trong model 
        ...
        //dùng để thêm dữ liệu vào bảng Auctions
        //AddRange dùng để thêm 1 danh sách các bản ghi vào bảng
        context.AddRange(auctions);
        //dùng để lưu dữ liệu vào database 
        context.SaveChanges();
        //vì sao khi dùng SeedData để lưu dữ liệu vào phương thức AuctionDbContext thì sẽ lưu dữ liệu của aucion và item vào database?
        //vì trong class AuctionDbContext đã khai báo DbSet<Auction> Auctions { get; set; } và navigation properties trong class Item.cs
